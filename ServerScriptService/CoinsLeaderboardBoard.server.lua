-- ServerScriptService/CoinsLeaderboardBoard.server.lua
-- Put this Script under Workspace.CoinsLeaderboard.Model.Scoreboard (replace globalLeaderboard).

local DataStoreService = game:GetService("DataStoreService")
local Players = game:GetService("Players")

local STORE_NAME = "LeaderboardCoins"
...
local function updateBoard(scroller: ScrollingFrame, data)
	...
	if not template then
		warn("[CoinsLeaderboard] Missing Frame template under script.")
		return
	end
...
while true do
	...
	if not scroller then
		warn("[CoinsLeaderboard] No SurfaceGui/ScrollingFrame found under", script.Parent:GetFullName())
		task.wait(5)
		continue
	end
	...
	if ok and page then
		...
		warn("[CoinsLeaderboard] Update failed:", page)
	end
